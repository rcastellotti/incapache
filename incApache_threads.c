...
void join_all_threads(int conn_no)
{
	...
	if(to_join[conn_no] != NULL)
	{
		...
		if(pthread_join(thread_ids[i], NULL) == -1)
			fail_errno("incapache: error in pthread_join");
		if(--no_response_threads[conn_no] == 0)
			++no_free_threads;
	...
	if(to_join[thrd_no] != NULL)
	{
		...
		if(i >= MAX_CONNECTIONS)
		{
			...
			if(pthread_mutex_lock(&threads_mutex) != 0)
				fail_errno("incapache: error while locking threads_mutex");
			if(--no_response_threads[conn_no] == 0)
				++no_free_threads;
			connection_no[i] = FREE_SLOT;
			if(pthread_mutex_unlock(&threads_mutex) != 0)
				fail_errno("incapache: error while unlockin threads_mutex");
		}
	}
	/*** TO BE DONE 4.1 END ***/
...
char* get_mime_type(char* filename)
{
	...
	if(pthread_mutex_lock(&mime_mutex) != 0)
		fail_errno("incapache: error while locking mime_mutex");
	/*** TO BE DONE 4.0 END ***/
	...
	if(pthread_mutex_unlock(&mime_mutex) != 0)
		fail_errno("incapsche: error while unlocking mime_mutex");
	/*** TO BE DONE 4.0 END ***/
